...
## Limite de confidencialidade a ter em mente

O conteúdo de cada treinamento vive em páginas HTML publicadas no GitHub Pages, que é **público por padrão**. A restrição de "equipe X não vê material de equipe Y" é aplicada pelo Google Sites não exibir o link para quem não tem permissão — não por uma barreira técnica no próprio GitHub Pages. Alguém com a URL exata de uma página de módulo consegue abri-la mesmo sem estar no grupo correspondente.

Isso é aceitável para o conteúdo atual (processos internos, sem dados de clientes), desde que:
- a URL do GitHub Pages nunca seja divulgada diretamente aos colaboradores (só o link do Google Site);
- nenhum dado de cliente, valor financeiro sensível ou informação que exija confidencialidade real seja colocado nessas páginas.

Se algum conteúdo futuro precisar de confidencialidade garantida de verdade, ele deve morar como página nativa dentro do próprio Google Sites (que aplica a permissão no nível do Google, de forma real) em vez de no GitHub Pages.
